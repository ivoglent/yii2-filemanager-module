...
(function ($, Modernizr, image_editor)
{
	...
	var FileManager = {
			...
			jQuery('#filter-input').on('keyup', function ()
			{
				...
				if (js_script)
				{
					delay(function ()
					{
						...
						$.ajax({
							url: "/ajax?action=filter&type=" + val
						}).done(function (msg)
						{
							if (msg != "")
							{
		...
		makeUploader: function()
		{
			jQuery('#fileupload').fileupload({
				...
				url: fileManagerConfigs.baseUrl + '/upload',
			...
			jQuery('#fileupload').bind('fileuploadsubmit', function (e, data) {
				...
				data.formData[fileManagerConfigs['tokenName']] = jQuery('#csrf-token').val();
			});
			// Load existing files:
		...
		uploadURL: function()
		{
			jQuery('#uploadURL').on('click',function(e){
				...
				data[fileManagerConfigs['tokenName']] = jQuery('#csrf-token').val();
				$.ajax({
					type: "POST",
					url: fileManagerConfigs.baseUrl + "/upload",
					data: data
		...
		makeSort: function(js_script)
		{
			...
			jQuery('.sorter').on('click', function ()
			{
				...
				{
					$.ajax({
						url: "/ajax?action=sort&sort_by=" + _this.attr('data-sort') + "&descending=" + (sortDescending ? 1 : 0)
					});
	...
	jQuery(document).ready(function ()
	{
		...
		jQuery('.new-folder').on('click', function ()
		{
			bootbox.prompt(jQuery('#insert_folder_name').val(), jQuery('#cancel').val(), jQuery('#ok').val(), function (name)
			{
				if (name !== null)
				{
					...
					$.ajax({
						type: "POST",
						url: fileManagerConfigs.baseUrl + "/execute?action=create_folder",
						data: data
					}).done(function (msg)
					{
						if(msg){
		...
		jQuery('.view-controller button').on('click', function ()
		{
			...
			$.ajax({
				url: "/ajax?action=view&type=" + _this.attr('data-value')
			}).done(function (msg)
			{
				if (msg != "")
				{
				...
				},
				{
					...
					"callback": function ()
					{
						...
						if (newFileName !== null)
						{
							...
							$.ajax({
								type: "POST",
								url: fileManagerConfigs.baseUrl +  "/execute?action=create_file",
								data: data
							}).done(function (status_msg)
							{
								if (status_msg != "")
	...
	function edit_text_file($trigger)
	{
		...
		$.ajax({
			type: "POST",
			url: "/ajax?action=get_file&sub_action=edit&preview_mode=text",
			data: { path: full_path }
				...
				[
					...
					},
					{
						...
						"callback": function ()
						{
							...
							$.ajax({
								type: "POST",
								url: fileManagerConfigs.baseUrl + "/execute?action=save_text_file",
								data: data
							}).done(function (status_msg)
							{
								if (status_msg != "")
					...
					},
					{
						...
						"callback": function ()
						{
							...
							if (newPerm != '' && typeof newPerm !== "undefined")
							{
								...
								$.ajax({
									type: "POST",
									url: fileManagerConfigs.baseUrl + "/execute?action=chmod",
									data: data
								}).done(function (status_msg)
								{
									if (status_msg != "")
	...
	function clear_clipboard()
	{
		bootbox.confirm(jQuery('#lang_clear_clipboard_confirm').val(), jQuery('#cancel').val(), jQuery('#ok').val(), function (result)
		{
			if (result == true)
			{
				$.ajax({
					type: "POST",
					url: "/ajax?action=clear_clipboard",
					data: {}
				}).done(function (msg)
				{
					if (msg != "")
	...
	function paste_to_this_dir(dnd)
	{
		bootbox.confirm(jQuery('#lang_paste_confirm').val(), jQuery('#cancel').val(), jQuery('#ok').val(), function (result)
		{
			if (result == true)
			{
				...
				$.ajax({
					type: "POST",
					url: fileManagerConfigs.baseUrl + "/execute?action=paste_clipboard",
					data: data
				}).done(function (msg)
				{
					if (msg != "")
...
// because of feedback and on error bahhhhh...
	...
	function execute_action(action, file, name, container, function_name)
	{
		if (name !== null)
		{
			...
			$.ajax({
				type: "POST",
				url: fileManagerConfigs.baseUrl + "/execute?action=" + action,
				data: data
			}).done(function (msg)
			{
				if (msg != "")
