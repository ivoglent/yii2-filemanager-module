...
try{
    ...
    $userDir = $baseUploadPath;
    $maxSize = isset($user->storage) ? $user->storage : 100 * 1024 * 1024;
    if (folderSize($userDir) + $config['MaxSizeUpload'] > $maxSize) {
        throw new \Exception(\Yii::t('app', 'No space for you. Please contact administrators for more information.'));
    }
